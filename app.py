...
@app.get("/pie_chart")
def get_pie(request: Request, amount: int = 10):
    top_ten = set(
    ...
    return templates.TemplateResponse(request, "pie_chart.html", context)


@app.get("/bad_actors")
def get_bad_actors(request: Request, amount: int = 10):
    plot_df = df.sort_values("Filesize", ascending=False).head(amount)
    fig = px.bar(plot_df, "Path", "Filesize")
    fig.update_xaxes(tickangle=45)
    context = {"bad_actors_chart": pio.to_html(fig, full_html=False)}
    return templates.TemplateResponse(request, "bad_actors.html", context)


@app.get("/read_stats")
async def read_stats(request: Request, directory: str = "/home/chase/Projects/"):
    get_stats(directory)
